...
| 12 | no safe pure candidate or still boundary-blocked |

The 12 remaining rows have no promoted candidate. A runtime gate passing is
not treated as proof that one of their custom helpers is hot or safe to
rewrite. Their matrix rows retain a concrete next action.

## Remaining rows

| Path | Current disposition | Workload/boundary evidence | Why no promotion |
| --- | --- | --- | --- |
| `dlls/dwrite/freetype.c` | triage | CEF x86_64 OSR; graphics P8 | Apple FreeType fallback is startup-only `dladdr`/path loading; no hot pure leaf isolated |
| `dlls/msvcrt/locale.c` | triage | P8 ARM64EC and all-architecture smoke | Custom hunk is diagnostic-only logging; removing or changing it is not a runtime speed win |
...
| `dlls/ntdll/unix/system.c` | triage | P8 ARM64/ARM64EC startup | Darwin LSE `sysctl` publication is a small startup query, not a measured hot path |
| `dlls/win32u/freetype.c` | triage | CEF OSR and graphics P8 | Bundled FreeType fallback and Fontconfig null guard are startup/availability paths |
...
This audit leaves the 12 remaining rows open for a future workload that
isolates a pure leaf. It does not authorize whole-file rewrites, pointer or
lock transformations, TLS changes, FEX callback changes, or semantic
ordering changes.
