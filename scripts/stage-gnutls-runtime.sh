...
test -f "$STAGE/secur32.so" || { echo "Missing Wine secur32.so: $STAGE/secur32.so" >&2; exit 1; }
mkdir -p "$STAGE"
...
/usr/bin/otool -l "$STAGE/secur32.so" | awk '
  /LC_RPATH/ { rpath = 1; next }
  rpath && /path @loader_path\// { found = 1 }
  rpath && /path / { rpath = 0 }
  END { exit !found }
' || {
  echo "secur32.so does not search its loader directory for GnuTLS" >&2
  exit 1
}

printf 'Staged %u relocatable ARM64 GnuTLS dylibs in %s\n' \
  "${#sources[@]}" "$STAGE"
